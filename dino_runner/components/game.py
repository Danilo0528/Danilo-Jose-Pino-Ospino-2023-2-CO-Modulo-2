...
class Game:
    ...
    def draw_power_up_time(self):
        if self.player.has_power_up:
            time_to_show = ((self.player.power_time_up - pygame.time.get_ticks)/1000,2)
            if time_to_show >= 0:
                self.menu.draw(self.screen, '{self.player.power_time.capitalize()} enable for {time_to_show} seconds', 500,50)
            else:
